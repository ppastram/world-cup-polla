...
import ExactScorePopup from './ExactScorePopup';
...
export default function MatchCard({ match, prediction, showPrediction = false, isGroupStage = false }: MatchCardProps) {
  ...
  return (
    <>
      ...
      >
        ...
        {/* Exact Score indicator for finished group stage matches */}
        {isFinished && isGroupStage && (
          <div className="mt-3 pt-3 border-t border-wc-border">
            <button
              onClick={() => setShowExactPopup(true)}
              className="flex items-center gap-1.5 text-xs text-gray-500 hover:text-gold-400 transition-colors mx-auto"
            >
              <Target className="w-3.5 h-3.5" />
              <span>{t("matches.exactScores")}</span>
            </button>
          </div>
        )}
      </div>
      ...
      {showExactPopup && (
        <ExactScorePopup
          ...
          isOpen={showExactPopup}
          onClose={() => setShowExactPopup(false)}
        />
      )}
    </>
  );
