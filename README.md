...
- **Payment flow** — receipt upload with admin verification (Nequi integration)
...
2. Create `.env.local`:
   ...
   NEXT_PUBLIC_NEQUI_NUMBER=your-nequi-number
   FOOTBALL_DATA_API_KEY=your-api-key
   CRON_SECRET=your-cron-secret
   RESEND_API_KEY=your-resend-key
   ```

3. Apply database migrations in order against your Supabase project (SQL editor or Supabase CLI).
...
Designed for **Vercel** with zero additional configuration. Environment variables must be set in the Vercel dashboard. The cron endpoint (`/api/cron/sync-matches`) can be triggered via Vercel Cron or an external scheduler, protected by the `CRON_SECRET` env var.
